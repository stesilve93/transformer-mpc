...
mu_LR = 5.593616014093049e05

N = 2

Cbar = np.zeros((N + 1, N + 1), dtype=float)
Sbar = np.zeros((N + 1, N + 1), dtype=float)
...
class PhysicsParams(nn.Module):
    ...
    def __init__(
        ...
        self.omega_raw = nn.Parameter(torch.tensor(float(omega)))
...
def physics_residuals_with_impulses(
    ...
    # Physical residuals
    rx_phys = a_fd[..., 0] - (2.0 * Om * vy + (Om * Om) * x + aT[..., 0])  # (B, L-1)
    ry_phys = a_fd[..., 1] - (-2.0 * Om * vx + (Om * Om) * y + aT[..., 1])  # (B, L-1)
    rz_phys = a_fd[..., 2] - (aT[..., 2])  # (B, L-1)
...
def vector_loss(pred, true):
    result = F.mse_loss(pred, true)
    return result


def vector_loss_weights(pred, true, w):
    """
    pred, true: (B, L, D)
    w: time weights -> (1, L, 1) or (L,) or (1, L)
       (higher values on the earliest instants)
    """

    # Per-time-step MSE
    per_t = F.mse_loss(pred, true, reduction="none").mean(dim=2)  # (B, L)
    ...
    # apply time weights and average over the batch
    return (per_t * w).sum(dim=1).mean()
