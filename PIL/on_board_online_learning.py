...
def main():
    ...
    # Bounds
    # -----------------------------------------------------------------------
    u_max = 0.6 / units.CU  # adim, per component
            ...
            while True:
                ...
                if ONLINE_TRAINING:
                    ...
                    if len(controls_hist) >= Lw:
                        ...
                        U_win = cyclic_window(np.asarray(controls_hist), start=i0, length=Lw)

                        # State sequence of the considered trajectory
                        X_seq = cyclic_window(np.asarray(traj_followed), start=i0, length=Lw + 1)
                    ...
                    if (
                        ...
                        if not hasattr(net, "optimizer"):
                            net.make_optimizer(lr=float(ONLINE_LR), weight_decay=0.0)
                        ...
                        if not FAST_BUT_REP:
                            ...
                            for x0_b, U_b, Yd_b in batches:
                                loss_online_iteration = net.train_step(x0_b, U_b, Yd_b, fast_but_rep=FAST_BUT_REP,grad_clip=1.0)
                            ...
                            for _ in range(ONLINE_STEPS_PER_UPDATE):
                                ...
                                loss_online_iteration = net.train_step(x0_b, U_b, Yd_b)
                                loss_online_single_step.append(loss_online_iteration)
                ...
                if (k >= 101):
                    ...
                    if k >= 120:
                        moving_window_2 = 20
                        MPE_pos_2 = np.asarray(MPE_current_vect[-moving_window_2:]).mean()
                    ...
                    if k >= (START_INSTANT_TRAINING+250):
                        if MPE_current_mean_vect[-1] < BEST_MPE:
                            ...
                            if number_iterations == PATIENCE:
                                print(f"Early stopping! Iteration number: {k} | Time [min]: {k * 5}")
