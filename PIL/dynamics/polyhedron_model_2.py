...
from Python_MPC.Test.sh_accelerations import sh_gravity_accel_cart_phi_torch_batch
...
def ROP2BP_dynamics_general(
    ...
    # Position and velocity in body-fixed frame (same indexing as your original code)
    r = x[[0, 2, 4]]  # [x, y, z]
    v = x[[1, 3, 5]]  # [vx, vy, vz]
    ...
    # Accelerations (Spherical Harmonics)
    ...
    acc = sh_gravity_accel_cart_phi_torch_batch(pos=pos)
    acc = acc.numpy()
    acc = acc.reshape(-1)
...
def dynamics_propagator_body_fast_general(
    ...
    if not sol_int.success:
        raise RuntimeError(f"solve_ivp failed: {sol_int.message}")
    #xx = np.asarray(sol_int.y, dtype=float).T     # added this
    xx = sol_int.y.T
    return sol_int.t, np.column_stack(
        (xx[:, 0], xx[:, 2], xx[:, 4], xx[:, 1], xx[:, 3], xx[:, 5])
    )
