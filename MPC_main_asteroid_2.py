...
def main():
    ...
    # Configuration
    # -----------------------------------------------------------------------
    model_path = SCEN_ROOT / "net_model" / "best_test_model_no_rot_no_relu_NET_asteroid_2.pt"
    ...
    traj_mat = (
        SCEN_ROOT / "reference_trajectories" / "trajectory_reference_asteroid_2.mat"
    )
    tt_mat = SCEN_ROOT / "reference_trajectories" / "time_reference_asteroid_2.mat"
    if VISUALISATION:
    ...
    """ Check --------------------------------------------------------------
    radius_orbit = 30_000
    mass_eros = 6.687 * 10 ** (15)
    mu_eros_estimated = mass_eros * G
    T_ipotesi = 2 * np.pi * np.sqrt(radius_orbit**3 / mu_eros_estimated)
    print(f"Periodo orbitale: {T_ipotesi:.2f} s ({T_ipotesi / 60:.2f} minuti)")

    T_estimated = 4 * 60  # dal obit
    """
    ...
    print("TT_VECT: ",tt_vect)
    x0 = traj_adim[0].copy()

    # -----------------------------------------------------------------------
    # Neural network
    # -----------------------------------------------------------------------
    net = NeuralNetwork(CONFIG["model_path"])
    net.load_model()
    ...
    # -----------------------------------------------------------------------
    if VISUALISATION:
        zoom_in_meters = 5_000
        # zoom_in_meters_follow = 16_000
        zoom_in_meters_follow = 100
        vis = Visualizer3D(
            units=units,
            ...
            win_back=60,
            win_ahead=60,
            follow=True,  # True: the live plot follows the current state
        )
