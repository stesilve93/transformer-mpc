...
def mesh_volume(vertices: np.ndarray, faces: np.ndarray) -> float:
    """
    Volume of a closed triangular mesh.
    vertices: (N,3)
    faces: (M,3)
    """
    ...
    vol_signed = np.einsum("ij,ij->i", v0, np.cross(v1, v2)).sum() / 6.0
    return float(abs(vol_signed))
...
class DynamicsPropagator:
    ...
    def compute(self, x0_adim: np.ndarray, U_vec_adim: np.ndarray, tt_win: np.ndarray):
        ...
        tt_win --> time window in which we have the propagation
        """
        L = len(U_vec_adim) // 3
...
class NeuralNetwork:
    ...
    def build_model(self, time_instants: int = 16):
        ...
        B = 1
        L = 15
        src_state = torch.zeros(B, 6, device=self.device, dtype=torch.float32)
        src_variations = torch.zeros(B, L, 3, device=self.device, dtype=torch.float32)
        summary(net, input_data=(src_state, src_variations), depth=3)
    ...
    def load_model(self):
        ...
        reference_model = torch.load(self.model_path, map_location=self.device)
...
class Optimization:
    ...
    def loss_and_grad_linear(
        ...
        L = U_vec.size // 3  # number of commands
        if U_vec.size != 3 * L:
            raise ValueError(
        ...
        def objective_only(U_flat: np.ndarray) -> float:
            ...
            for j in range(L):
                ...
                if idx < 0 or idx >= A_vect_AD.shape[2]:
                    raise IndexError(
                        f"Indice A/B out of range: idx={idx}, max={A_vect_AD.shape[2] - 1}"
                    )
                A_k = A_vect_AD[:, :, idx]
                B_k = B_vect_AD[:, :, idx]
...
class MPC:
    ...
    def state_rotation(self, x: np.ndarray, R: np.ndarray) -> np.ndarray:
        """
        Rotate a state (6,) or a control (3,) using the matrix R (3x3).
    ...
    def run(
        ...
        for k in range(1, T_steps_ref + 1):
            ...
            # Here is modified the reference trajectory inside the optimizer
            ...
            nlp.add_option("max_iter", int(self.options_solv.get("max_iter", 2000)))
            nlp.add_option("hessian_approximation", "limited-memory")
            nlp.add_option("print_level", int(self.options_solv.get("print_level", 5)))
            ...
            if self.scenario == "Neural_Network":
                ...
                # N_prop --> number of time instants to propagate (excluding the initial state)
